...
def cargar_imagenes():
    ...
    if procesar:
        ...
        ancho = 400
        with col1:
            st.image(imagen_gotas, caption=f"", width=ancho)
        with col2:
            st.image(imagen_hoja, caption=f"", width=ancho)

        # Mostrar porcentaje de recubrimiento
        color = "green" if porcentaje <= 20 else "orange" if porcentaje <= 50 else "red"
        barra_html = f"""
        <div style='margin-top: 20px;'>
            <div style='font-size:40px; font-weight:bold; text-align:center;'>Recubrimiento de cobre: {porcentaje:.2f}%</div>
            <br>
        </div>
        """
